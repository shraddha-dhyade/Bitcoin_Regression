...
def computeDelta(wt, X, Xi):
    ...
    for i in range(0,len(Xi)):
        Xi_row = Xi.iloc[i]
        term = np.exp(wt*computeSimilarity(X.iloc[:len(X)-1], Xi_row.iloc[:len(Xi_row)-1]))
        sum_numerator = sum_numerator + Xi_row[-1]*term
        sum_denominator = sum_denominator + term

    delta = sum_numerator/sum_denominator    
    return delta

def computeSimilarity(a,b):
    sum = 0
    for i in range(0,len(a)):
        sum = sum + ((a.iloc[i] - np.mean(a))*(b.iloc[i] - np.mean(b)))

    return sum/(len(a)*np.std(a)*np.std(b))
